...
1. **Количество точек**: КРИТИЧНО ВАЖНО!
   - 30 минут → 4-6 точек (генерируем с запасом 200%)
   - 1 час → 6-8 точек
   - 2 часа → 8-10 точек
   - 3 часа → 10-14 точек
   - Полдня (4-5 часов) → 14-18 точек
   - Целый день (6+ часов) → 20-24 точки

   **ВАЖНО:** Генерируй больше точек чем нужно (200% от желаемого), так как часть точек может не пройти валидацию координат.
   Система автоматически отфильтрует точки с неточными координатами и оставит нужное количество.
...
8. **search_queries (КРИТИЧНО для валидации!):**
   Для каждой точки создай массив из 2-4 альтернативных названий для поиска в 2GIS:
   - Полное официальное название
   - Сокращенное название (например, "Кремль" вместо "Астраханский кремль")
   - Название с типом объекта (например, "Музей краеведения", "Парк Победы")
   - Альтернативное местное название если есть

   Примеры:
   - "Собор Святого Владимира" → ["Собор Святого Владимира", "Кафедральный собор", "Владимирский собор"]
   - "Лебединое озеро" → ["Лебединое озеро", "озеро Лебединое", "парк у озера"]
   - "Кафе Пушкин" → ["Кафе Пушкин", "Пушкин кафе", "Pushkin cafe"]
